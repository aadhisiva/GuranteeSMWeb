...
export default function AssignedData() {
  ...
  const getAllMaster = async () => {
    setLoading(true);
    let res = await postRequest("getAllAssignedUsers", "");
    if (res?.code === 200) {
  ...
  const handleModifyAssignedUser = async (values: IMasterData) => {
    let res = await postRequest("modifyRefractionist", values);
    if (res.code === 200) {
      setEditForm(false);
      await getAllMaster();
    } else {
  ...
  const rednerForm = () => {
    return (
      <ModalFormEdit
        ...
        handleModifyAssignedUser={handleModifyAssignedUser}
